...
.smallCbioList = function() {
  glioRTK = c(
   ...
   symlist = list(rasraf=rasraf, glioRTK=glioRTK,
      pi3k=pi3k,
      ovtumsupp = ovtumsupp)
   cbioNames= c(  glioRTK="Glioblastoma: RTK/Ras/PI3K/AKT Signaling (17 genes)",
      pi3k="General: PI3K-AKT-mTOR signaling (17 genes)",
      ovtumsupp = "Ovarian Cancer: Putative tumor-suppressor genes in epithelial ovarian cancer (16 genes)",
      rasraf = "General: Ras-Raf-MEK-Erk/JNK signaling (26 genes)")
   list(symlist = symlist, cbioNames=cbioNames[names(symlist)])
}

.upsetGSC.getmat = function(gsc, ...) {
  allg = unique(unlist(geneIds(gsc)))
  mat = matrix(0, nc=length(gsc), nr=length(allg))
  colnames(mat) = names(gsc)
  rownames(mat) = allg
  for (i in 1:ncol(mat)) {
    mat[match(geneIds(gsc[[i]]), allg),i] = 1
  }
  mat
}
